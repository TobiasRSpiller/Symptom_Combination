...
# Import data
data_MDD_raw <- read_excel("../Raw_data/Ilan_PHQ9_and_GAD7_and_MH_Project_051223.xlsx")
```
...
data_MDD <- data_MDD %>%
  mutate(
    MDD_Diagnosis = ifelse(
      ...
      rowSums(across(starts_with("MDD_Symptom")) >= 2) >= 5,
      TRUE, 
      FALSE
    )
  )
...
# Table 1
data_MDD_table1 <- data_MDD %>%
  dplyr::select(AGE, GENDER, DEPRESSION_DX, MDD_Diagnosis, PHQ_9_Total) 
...
data1 <- data_MDD_subset %>%
  dplyr::select(MDD_Symptom1:MDD_Symptom9)
...
data_all_cronbach <- data_MDD %>% dplyr::select(MDD_Symptom1:MDD_Symptom9) 
...
data2 <- data1_binarized %>% 
  dplyr::select(total:ncol(data1_binarized)) %>% 
  dplyr::select(-total) %>% 
  tibble()
...
data_MDD_subset_selected <- data_MDD_subset %>% dplyr::select(MDD_Symptom1:MDD_Symptom9)
